...
pokeApp.compareUserToPokemon = (subject, value) => {
	...
	do{
		...
		let currentPokemonValue = pokeApp.pokemonInfoList[i][subject];
	...
	if (pokemonEven > 0) { // Condition if there is a pokemon who is the same as the user
		...
		$("#resultsOver__pokemonName").text(pokeApp.pokemonInfoList[lastPokemon].name);
		...
		$("#resultsUnder__pokemonName").text(pokeApp.pokemonInfoList[pokemonUnder].name);
		...
		$("#resultsOver__pokemonName").text(pokeApp.pokemonInfoList[pokemonOver].name);
...
pokeApp.setBackground = () => { // Used to randomly generate pokemon in the background
	...
	for(let i = 0; i <= numberOfPokemon; i++) { // runs a loop to randomly generate 50 pokemon into the background
		...
		let randomPokemonSize = Math.floor((Math.random() * 120) + 100);
